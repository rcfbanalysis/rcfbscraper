...
def Convert_PBP_Data(pbp_file):
	...
	for play in pbp_data:
		if play[2] != "":
			(visitor_code, visitor_name, abbv_arr) = Define_Team_Abbreviation(pbp_data, play[2], teams, abbv_arr)
			(home_code, home_name, abbv_arr) = Define_Team_Abbreviation(pbp_data, play[3], teams, abbv_arr)
			break
	...
	for play in pbp_data:
		...
		if m:
			(code, name, abbv_arr) = Define_Team_Abbreviation(pbp_data, m.group("team"), teams, abbv_arr)
...
def Define_Team_Names(pbp_data, team_arr, abbv_arr):
	...
	for play in pbp_data:
		...
		if m:
			(code, name, abbv_arr) = Define_Team_Abbreviation(pbp_data, m.group("offense"), team_arr, abbv_arr)
			if team1_code == 0:
				team1_code = code
				team1_name = name
			elif team2_code == 0:
...
pbp_file = "pbp Data/boisemiss.csv"
Convert_PBP_Data(pbp_file)

# Read new play-by-play
pbp_data = Read_CSV("2014 Stats/play-by-play.csv")

# Create game and collect drive data
game = Game(pbp_data)
drives = Compile_Drives(pbp_data, game)

# Parse data into play format
plays = []
game.Current_Qrt = 1
...
Write_CSV(drive_data, "2014 Stats/drive.csv")
...
Write_CSV(play_data, "2014 Stats/play.csv")
