...
def Convert_PBP_Data(pbp_file):
	...
	for line in pbp_data[1:]:
		...
		if len(line[C.DIR]) > 0 and line[C.DIR] != "ERROR":
			...
			line[C.DIR] = number
		# find team penalty
		if len(line[C.PENTM]) > 0:
			(number, name, abbv_arr) = Find_Abbv_Team(pbp_data, line[C.PENTM], teams, abbv_arr)
			line[C.PENTM] = number
		# find defense
		...
		line[C.OFF] = number
		# fix position
		if line[C.DIR] != line[C.OFF] and line[C.DIR] != line[C.DEF] and line[C.TYPE] != "KICK" and line[C.TYPE] != "EXTRA POINT" and line[C.TYPE] != "PENALTY":
			#print "These teams don't know where they're going!\n"
			#print "TYPE: " + line[C.TYPE]
			#print "OFF:  " + str(line[C.OFF])
			#print "DEF:  " + str(line[C.DEF])
			#print "DIR: "  + str(line[C.DIR])
			#raw_input()
			line[C.DIR] = "ERROR"
		if line[C.OFF] == line[C.DIR]:
			line[C.YARD] = 100 - int(line[C.YARD])
		# parse game date
